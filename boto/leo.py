...
mimeType = magic.open(magic.MAGIC_MIME)
...
try:
    ...
    file_digest = hashlib.md5(fileObject.read()).hexdigest()
    ...
    thisfileObject = open(this_file_path, "r")
    this_file_digest = hashlib.md5(thisfileObject.read()).hexdigest()
    this_file_size = os.path.getsize(this_file_path)
    thisfileType = mimeType.file(this_file_path)
